...
LEFT JOIN company_dim ON job_postings_fact_staging.company_id = company_dim.company_id
WHERE job_postings_fact_staging.job_title_short = 'Data Analyst' AND job_postings_fact_staging.job_location ILIKE ('Anywhere') AND job_postings_fact_staging.salary_year_avg IS NOT NULL AND job_postings_fact_staging.job_work_from_home = TRUE
GROUP BY title, location, yearly_salary, company_name, type, id, remote
